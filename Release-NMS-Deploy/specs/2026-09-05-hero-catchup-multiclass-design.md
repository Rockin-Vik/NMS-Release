...
| D3 | **Clamp, then route, then let the stock loop run.** | The max-level clamps rewrite the target after the derivation loop (`exp.cpp:1254-1270`) and the pool is written at `1319`. Routing before those clamps would persist rows above the cap. So: after the Lua `SetEXP` hook (`1045-1058`) and the group incentive bonus (`1136-1142`) have finished mutating the target, compute the clamped target (`MaxExpLevel` 70, per-character max), route it into rows, set the pool to the minimum row, then the stock loop runs. After a route, D23 clamps `set_exp` and the derived level down to 70 and does not let `KeepLevelOverMax` write the watermark. The derivation and clamp logic is extracted into helpers. The delta is taken against the pre-repair pool because the target was derived from that same pool. |
...
| D23 | **Hard cap is `MaxExpLevel` (70).** After a route, clamp `set_exp` and the derived level down to that cap. Do not let `KeepLevelOverMax` write `GetEXPForLevel(GetLevel()+1)` (the watermark). `GetClassExpCap` is the 70-exp ceiling, so an 84 row comes down to 70 rather than staying as a watermark. Login clamps every class row to that ceiling, then sets the pool to the trailing row. `KeepLevelOverMax` defaults off; `MaxLevel` defaults to 70. | Live B3 (2026-09-07): pool `1837382400` vs row `1398659552` was KeepLevelOverMax lifting an over-cap body. The repair stays; over-cap progress is discarded down to 70. |
...
// stock derivation loop runs, then D23: clamp set_exp and check_level down to
// MaxExpLevel (70). Do not raise set_exp to GetEXPForLevel(GetLevel()+1).
```
