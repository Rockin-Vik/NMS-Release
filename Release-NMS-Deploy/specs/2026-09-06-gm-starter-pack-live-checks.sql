...
-- Section A runs BEFORE merging/deploying; sections B and C after world has applied custom v31/v32
...
-- pick the next free id in the block and change BOTH the v31 entry and the v32 'box' seed row.
SELECT id, Name, bagslots, bagsize, bagwr, bagtype, nodrop, norent
FROM items
WHERE id BETWEEN 9011000 AND 9011999
ORDER BY id;

-- Expect: exactly one row, the Legendary sword (2017731 in the release dump). The v32 seed picks
