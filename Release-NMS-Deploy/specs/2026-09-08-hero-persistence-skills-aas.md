...
| `NMS_multiclass_utils.pl` 44-48 `CommonCharacterUpdate` | zone-in, connect, level-up, after `AddClass` | `SetSkill(53, 0)` when `MaxSkill(Tracking) == 0` and the Perl `GetSkill` (the same clamped virtual) is non-zero. A no-op **only while catching up**; with equal class rows the clamp is off (`client.cpp` 15182), the raw value shows, and the write is live on every zone-in: drop the last tracker while catching up, finish the climb, zone, and Tracking goes to 0 | remove (D2) |
...
- **What D2 actually removes:** the C++ drop loop (`client.cpp` 15104-15109) tests `!CanHaveSkill(skill) && GetSkill(skill)`. Today that write runs only on a drop with equal class rows (no catch-up, so no clamp); under D1 it can never run, because D1's `GetSkill` returns 0 for every `!CanHaveSkill` skill in every state. D2 deletes it as dead code. The live parts of D2 are the combat-ability gate above and the plugin's Tracking write, which reads the same `GetSkill` but through Perl on every zone-in and is live whenever the hero is not catching up (§1.1). §4 step 4 runs a same-level drop so that path is exercised, not assumed.
...
`Client::ResetAA` (`aa.cpp` 548-573) on a character with `Custom:MulticlassingEnabled` refunds **from the rows, not from memory, and not through the loader**. Two earlier drafts of this decision were wrong in ways worth recording: v2 summed the rows and then also called `RefundAA`, which paid held ranks twice and paid for grant-only and expended-charge ranks that `RefundAA` skips (888, 884); v3 said "load every row without the `CanUse` filter, then `RefundAA` once", but the per-client loader's only write is `SetAA` (1843), and `SetAA` refuses any value above `GetMaxLevel(this)` (1939-1941), which walks the ranks until `CanUseAlternateAdvancementRank` fails (`aa_ability.cpp` 45-51) and is therefore **0 for every shelved ability** (rank 1 fails the class test, 1982-1986). A shelved rank can never enter `aa_ranks` through that path, so v3's refund would have paid nothing for it and `DeleteCharacterAAs` (572) would still have deleted it: the exact hole D8 exists to close.

So: factor the per-rank rule out of `RefundAA` into one helper, `RefundableCost(ability, rank, charges)` (0 for grant-only, 0 for an expendable ability with no charges left, else `rank->total_cost`), and use it in both places. `RefundAA` keeps its loop over `aa_ranks` for the single-class path. Under multiclassing `ResetAA` instead reads every `character_alternate_abilities` row for the character through the repository, resolves each to `zone->GetAlternateAdvancementAbility(aa_id)` and `GetRankByPointsSpent(aa_value)`, adds `RefundableCost` for each, then clears memory (`ClearAAs`, the `memset`) and calls `DeleteCharacterAAs` as today. Nothing goes through `SetAA`. The refund equals what was paid for held and shelved ranks alike. All nine callers (§1) inherit it, and the timer spec's mapping wipe lands in the same function. The Ayonae AA reset copy (`Vision_of_Ayonae.pl` 240-263) does not change.
...
4. Drop MNK while catching up. **Expect:** `character_skills` unchanged; Monk-only skills greyed without a re-zone; Monk-only AAs absent from the window; row count unchanged; `aa_points` unchanged; the Monk passive's contribution gone from `#showstats`; Combat Fury still owned; buffs still up (D10 default); no "You have lost access" lines; gems above level 1 unmemorized (D11). **Then the same-level variant:** a hero whose class rows are equal (not catching up, `IsCatchingUp` false) drops its only tracking class, then zones. **Expect:** `character_skills` unchanged including Tracking, the AA rows unchanged, the dropped class's skills greyed. No gem change is expected here: the level does not move, so the D11 decrease hook does not run and no gem is above the hero level. Today this variant zeroes the dropped class's skills in C++ and Tracking in the plugin on the next zone-in; the catching-up variant zeroes neither, so only the pair proves both writes are gone.
...
7. `#resetaa` on the hero with MNK shelved. **Expect:** `aa_points` rises by the sum of held **and** shelved ranks' costs, grant-only and charge-expended ranks excluded as today; all rows gone (D8); the dynamic timer mapping rows gone (timer spec). The number to compare against is computed from the pre-reset rows with the §4 preamble's query, not from the AA window, which never showed the shelved ranks.
...
- `Release-NMS-Server/zone/aa.cpp`: delete `RefundUnusuableAA` (909-951); new `RefundableCost` helper factored out of `RefundAA` (870-906); `ResetAA` (548-573) refunds from the repository rows under multiclassing, never through `SetAA`; bounds checks at 566, 1149 and 1839; optional `level_req` fallback in `SendAlternateAdvancementRank` (§4 step 1).
- `Release-NMS-Server/common/repositories/character_alternate_abilities_repository.h`: the existing `GetWhere` by character id is enough; no new loader in `zonedb.cpp`.
