...
`not held. Add is free.<br>` (`:132`), `Select a class, then press Add Class or Remove Class.<br>` (`:135`) and `The guildmasters and the Vision of Ayonae in the Bazaar make the same changes.` (`:137`) stay.

**Shelved classes show their level (in-game pass finding, 2026-09-09).** The tab printed nothing for a class the character once held, although its `character_class_exp` row keeps the level and a re-add brings it back at that level; the tester read that as the level being lost. The bulk stats packet's sixteen `ClassLevelN` stats (`inventory.cpp`) now carry the row level for every class instead of 0 for a class not in the bitmask (the held set is the bitmask stat, unchanged), and the tab lists such a class in gold with its level and the status `Current`, with the info box line `<class>: current at level N, not in play. Add is free and brings it back at that level.` The words are the tester's (2026-09-09): `Active` for a class in play (was `Held`), `Current` for one set aside with its level kept (was `Shelved`), `never played. Add is free.` for a class with no row (was `not held`), and the header reads `Active classes: N`; "held" and "shelved" were developer words. No opcode or struct change; an older add-on ignores the extra levels because it only prints them for held classes. The add-on reads these rows through a raw stat reader (`NMS_GetClassLevelRaw`): the plain reader falls back to the on-screen level when a stat is 0, which is right for a class in play whose stat has not arrived and wrong for a class never played, and the first build showed every never-played class as `Current 1` (in-game finding). The client-side pre-checks (`:289-312`) contain no cost, lockout or zone logic and stay; the local last-class refusal (`:307-308`) stays.
...
**Results (local test server, RoF2 client, 2026-09-09; `HeroCatchupEnabled`, `MulticlassingEnabled` and `MaxMulticlasses` = 4 read from `rule_values`; the status-255 test account with `GM:MinStatusToZoneAnywhere` raised above it locally so `CanEnterZone` ran):** step 1 pass (three-class 70 hero added a fourth class in the Plane of Fear, floor 46: level 1, no zone refusal, no announcement in any zone or world log); step 2 pass (relogged into Fear at level 1, no kick); step 3 pass (a new single-class character was sent to bind on relog in Crescent Reach, floor 65, and the guild hall); step 4 pass (Hero tab remove at once with the new info box, tooltip and plugin line, "Welcome to level 70" on the way back up; no cost or lockout text; the plant was not needed because the fee code is gone); step 5 pass; step 6 pass for the feign branch (a 70 Monk feigned: remove refused with the red line, add refused with the cap line because he held four; aggro and duel not run, same expression); step 7 pass (Ayonae hail without the free-removal notice, one line per class with `Proceed`, removal on click); step 8 pass (last class refused through Ayonae's `remove_<id>`); step 9 not run (no guildmaster in a floored zone reachable safely; D3 is covered by step 1); step 10 pass (four-class hero printed 4 for held and 5 for the rest, a two-class character 4 and 0, never 8); step 11 pass in its relog form (a one-class former hero relogged into Fear at level 1 and stayed; `character_class_exp` kept every dropped class's row at its level). Found and fixed during the pass: the Hero tab printed nothing for a dropped class, see D7 shelved classes. Seen and left: the Plane of Fear is a bad test zone (zone-in aggro; and with the Spells log category on, NPC casting hits a stock `LogSpells` crash in `spells.cpp`, owner note); a client below a skill's class level never offers Feign Death whatever the stored value.
