...
bool SharedDatabase::GetSharedBank(uint32 id, EQ::InventoryProfile *inv, bool is_charid)
{
	...
	for (const auto& e : l) {
		...
		// The shared bank SAVED these and then threw them away on load. UpdateSharedBankSlot writes
		// color/ornament_icon/ornament_idfile/ornament_hero_model (see :481, :489-491) and this loader
		// restored only augments and custom_data -- so the columns were faithfully populated and never
		// read, and dye plus Hero's Forge ornamentation died on every shared-bank round trip.
		// (Note the abandoned `//SetTint(...)` at zone/inventory.cpp:2080 -- this was known about
		// and left.)
...
#undef F
	...
	for (auto& row = results.begin(); row != results.end(); ++row) {
		...
		// NMS Custom Stuff
		if (RuleB(Custom, UseNMSItemMutations)) {
			bool skip = false;
