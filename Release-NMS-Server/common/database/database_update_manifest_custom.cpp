...
ON DUPLICATE KEY UPDATE name = VALUES(name);
	...
	// ------------------------------------------------------------------------------------------------
	// Versions 27-28: FABLED SEASON schema (see Release-NMS-Deploy/FABLED-ENCOUNTERS.md 6.2 / 6.8).
	//
	// Two tables. `fabled_npcs` is the roster (content DB): one row per eligible npc_types.id with
	// the live Fabled level and optional per-row overrides. `fabled_season` is the operational state
	// world owns (player DB): exactly one row, id = 1, seeded inactive so a fresh server boots with
	// the feature off. Both entries are DDL + idempotent DML only (CREATE TABLE IF NOT EXISTS,
	// INSERT ... SELECT ... WHERE NOT EXISTS) and their checks test the artifact they create, never
	// the version stamp, so a half-applied server re-runs them and a healthy one no-ops.
	//
	// The roster CONTENT is deliberately not a migration: it is utils/sql/fabled_roster_seed.sql,
	// generated by utils/scripts/fabled_roster.py, applied like the other loose SQL files in
	// CODEBASE.md 4.4. The health check (utils/sql/nms_content_health_check.sql) prints its row count.
	//
	// Same rules as always: one entry per version, universal SQL only (no MariaDB-only syntax),
	// checks guard their own SQL, everything idempotent, no DROP/TRUNCATE/DELETE.
	// ------------------------------------------------------------------------------------------------

	// Roster table, content DB. Check = the table itself in information_schema of the CONNECTED
	// schema (DATABASE() is the content DB on this connection). CREATE TABLE IF NOT EXISTS keeps a
	// re-run clean. Multipliers default to -1 (= derive from the level delta), npc_spells_id 0
	// (= keep the NPC's own), chance 0 (= use the season chance).
	ManifestEntry{
		.version = 27,
...
CREATE TABLE IF NOT EXISTS `fabled_npcs` (
  ...
  KEY `era` (`era`)
	...
	// Season state row, player DB. Check = the SEED ROW, not the table: if the table exists but
	// the seed is missing (a run died between the two statements) the check reads empty and the
	// whole entry re-runs. While the table does not exist yet the check errors, which the runner
	// treats as an empty result (MySQL error logging is muted during the check pass), so a fresh
	// server runs it too. CREATE TABLE IF NOT EXISTS + INSERT ... WHERE NOT EXISTS make the re-run
	// clean; an operator's live season row is never overwritten.
	ManifestEntry{
		.version = 28,
		.description = "2026_09_05_fabled_season_state_row",
		.check = "SELECT id FROM fabled_season WHERE id = 1",
		.condition = "empty",
		.match = "",
		.sql = R"(
CREATE TABLE IF NOT EXISTS `fabled_season` (
  ...
  `scope_kind` TINYINT UNSIGNED NOT NULL DEFAULT 0,
  `scope_value` VARCHAR(32) NOT NULL DEFAULT '',
  `chance` TINYINT UNSIGNED NOT NULL DEFAULT 50,
  `loot_tier` TINYINT UNSIGNED NOT NULL DEFAULT 2,
...
CREATE TABLE IF NOT EXISTS `character_class_exp` (
	...
	ManifestEntry{
		...
		// check cannot look at `character_class_exp`: version 29 has not created it yet at that
