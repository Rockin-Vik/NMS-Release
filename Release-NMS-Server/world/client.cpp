...
bool Client::HandleCharacterCreatePacket(const EQApplicationPacket *app) {
	...
	if(OPCharCreate(char_name, cc) == false) {
		...
		DeleteCharacterByNameFromCache(char_name);
		auto outapp = new EQApplicationPacket(OP_ApproveName, 1);
		outapp->pBuffer[0] = 0;
		QueuePacket(outapp);
		safe_delete(outapp);
...
bool Client::HandleEnterWorldPacket(const EQApplicationPacket *app) {
	...
	// NMS: crash-reconnect group grace. A fresh (non-zoning) login used to silently
	// DELETE this character's group_id rows here, with no notification to any zone.
	// Remaining members' zones still held the group with this member's name, so after
	// a crash+relaunch the crasher had an empty group window while everyone else kept
	// a phantom entry that only a manual disband could clear. Every legitimate exit
	// already cleans these rows loudly in-zone (camp_timer/linkdead_timer/destructor
	// -> LeaveGroup -> DelMember/DisbandGroup, world_boot ClearGroup on reboot), so
	// the silent strip protected nothing. Keeping the rows means the zone-entry
	// rejoin path (Handle_Connect_OP_ZoneEntry) simply puts a returning crasher back
	// into their group - live-like reconnect grace. The leader hint below now goes to
	// grouped FRESH logins too, so they take the exact packet path zoning always took.
...
bool Client::HandleDeleteCharacterPacket(const EQApplicationPacket *app) {
	...
	if (char_acct_id == GetAccountID())	{
		...
		DeleteCharacterByNameFromCache((char *)app->pBuffer);
		SendCharInfo(m_selected_character_set);
	}
...
bool Client::HandleCharacterSetRequest(const EQApplicationPacket* app) {
	...
	LogCharacterSets("Account [{}] updated default character set to [{}] (last selected)", GetAccountID(), m_selected_character_set);
	return true;
}
...
bool Client::HandleCharacterSetCreateRequest(const EQApplicationPacket* app) {
	...
	bool create_set = !p->set_id;
	bool delete_set = p->set_id && !strlen(p->name);
	bool rename_set = p->set_id && strlen(p->name);
	...
	if (delete_set) {
		...
		if (p->set_id == m_selected_character_set && !m_character_sets.empty()) {
...
bool Client::HandleCharacterSetUnlockRequest(const EQApplicationPacket* app) {
	...
	switch (csur->type) {
	case CharacterSetUnlockAction::CHARACTER_SET:
		success = HandleSetUnlock(csur->quantity);
		break;
	case CharacterSetUnlockAction::CHARACTER_SLOT:
...
bool Client::HandlePacket(const EQApplicationPacket *app) {
	...
	switch(opcode)
	{
		...
		{
			return HandleCharacterSetCreateRequest(app);
		}
		case OP_CharacterSetMoveRequest:
		{
			return HandleCharacterSetMoveRequest(app);
		}
		case OP_CharacterSetUnlockRequest:
		{
			return HandleCharacterSetUnlockRequest(app);
		}
		default:
		{
...
bool Client::HandleChecksumPacket(const EQApplicationPacket *app)
{
	...
	// Record the client-reported checksums on the account for later verification.
	if (app->GetOpcode() == OP_World_Client_CRC1) {
		database.SetAccountCRCField(GetAccountID(), "crc_eqgame", cs->checksum);
		LogInfo("HandleChecksumPacket: Saved CRC1 to db for account ID {}: {}", GetAccountID(), cs->checksum);
	} else if (app->GetOpcode() == OP_World_Client_CRC2) {
		database.SetAccountCRCField(GetAccountID(), "crc_skillcaps", cs->checksum);
		LogInfo("HandleChecksumPacket: Saved CRC2 to db for account ID {}: {}", GetAccountID(), cs->checksum);
	}
...
void Client::SendCharacterSetInfo() {
	...
	if (sets.size() > MAX_CHARACTER_SETS) {
		sets.resize(MAX_CHARACTER_SETS);
	}
	...
	l->set_count = std::min(sets.size(), static_cast<size_t>(MAX_CHARACTER_SETS));
	...
	l->emperors_favor_available = m_emperors_favor_available;
	l->character_slot_cost = RuleI(Custom, EmperorsFavorUnlockCharacterSlotCost);
	l->character_set_cost = RuleI(Custom, EmperorsFavorUnlockCharacterSetCost);
...
void Client::PopulateCharacterDataCache() {
	...
	m_character_sets.clear();
	auto default_set = AccountCharacterSetsRepository::NewEntity();
	default_set.account_id = GetAccountID();
	default_set.set_id = 1;
	default_set.set_name = "Default";
	m_character_sets.push_back(default_set);

	m_character_set_members.clear();
	for (const auto& character : m_account_characters) {
		AccountCharacterSetMembersRepository::AccountCharacterSetMembers member{};
		member.account_id = GetAccountID();
		member.set_id = 1;
		member.character_id = character.id;
		m_character_set_members.push_back(member);
	}

	m_character_set_meta = AccountCharacterSetLimitsRepository::NewEntity();
	m_character_set_meta.account_id = GetAccountID();
	m_character_set_meta.default_set = 1;
	m_character_set_meta.eom_sets = 0;
	m_character_set_meta.bonus_sets = 0;
	m_character_set_meta.eom_slots = 0;
	m_character_set_meta.bonus_slots = 0;

	m_default_character_set = 1;
	m_selected_character_set = 1;

	m_emperors_favor_available = AccountAltCurrencyRepository::FindByAccountAndCurrency(database, GetAccountID(), EMPERORS_FAVOR_CURRENCY_ID).amount;
}
...
bool Client::AddCharacterToSetInCache(uint32 set_id, uint32 character_id) {
	...
	if (current_count >= EQ::constants::CHARACTER_CREATION_LIMIT) {
		return false;
	}
...
uint8 Client::GetMaxCharacterSets() {
	uint8 total = RuleI(Custom, BaseCharacterSets) +
		m_character_set_meta.eom_sets +
		m_character_set_meta.bonus_sets;
	return std::min(total, MAX_CHARACTER_SETS);
}
...
uint32 Client::GetAvailableSlotUnlocks() {
	int32 max_emperors_favor_slots = RuleI(Custom, EmperorsFavorUnlockCharacterSlots);
	if (max_emperors_favor_slots == -1) {
		return 999;
	}
	return (m_character_set_meta.eom_slots >= max_emperors_favor_slots) ? 0 : (max_emperors_favor_slots - m_character_set_meta.eom_slots);
}

uint32 Client::GetAvailableSetUnlocks() {
	int32 max_emperors_favor_sets = RuleI(Custom, EmperorsFavorUnlockCharacterSets);
	if (max_emperors_favor_sets == -1) {
		uint32 current_max = GetMaxCharacterSets();
		return (current_max >= MAX_CHARACTER_SETS) ? 0 : (MAX_CHARACTER_SETS - current_max);
	}
	return (m_character_set_meta.eom_sets >= max_emperors_favor_sets) ? 0 : (max_emperors_favor_sets - m_character_set_meta.eom_sets);
}
...
bool Client::HandleSetUnlock(uint32 quantity) {
	if (GetMaxCharacterSets() >= MAX_CHARACTER_SETS) {
		LogError("Account [{}] attempted to unlock character sets but already at maximum ({})", GetAccountID(), MAX_CHARACTER_SETS);
		return false;
	}

	if (RuleI(Custom, EmperorsFavorUnlockCharacterSets) != -1 && GetAvailableSetUnlocks() < quantity) {
	...
	uint32 cost_per_set = RuleI(Custom, EmperorsFavorUnlockCharacterSetCost);
	uint32 total_cost = quantity * cost_per_set;

	if (m_emperors_favor_available < total_cost) {
		LogError("Account [{}] attempted to unlock {} character sets for {} Emperor's Favor but only has {}",
			GetAccountID(), quantity, total_cost, m_emperors_favor_available);
		return false;
	}

	m_emperors_favor_available -= total_cost;
	AccountAltCurrencyRepository::UpdateByAccountAndCurrency(database, GetAccountID(), EMPERORS_FAVOR_CURRENCY_ID, m_emperors_favor_available);

	m_character_set_meta.eom_sets += quantity;
	// AccountCharacterSetLimitsRepository::UpdateAccountSetMeta(database, m_character_set_meta);

	LogCharacterSets("Account [{}] unlocked {} character sets for {} Emperor's Favor, now has [{}] unlocked sets",
...
bool Client::HandleSlotUnlock(uint32 quantity) {
	if (RuleI(Custom, EmperorsFavorUnlockCharacterSlots) != -1 && GetAvailableSlotUnlocks() < quantity) {
	...
	uint32 cost_per_slot = RuleI(Custom, EmperorsFavorUnlockCharacterSlotCost);
	uint32 total_cost = quantity * cost_per_slot;

	if (m_emperors_favor_available < total_cost) {
		LogError("Account [{}] attempted to unlock {} character slots for {} Emperor's Favor but only has {}",
			GetAccountID(), quantity, total_cost, m_emperors_favor_available);
		return false;
	}
	...
	LogCharacterSets("Account [{}] unlocked {} character slots for {} Emperor's Favor, now has [{}] unlocked slots",
...
bool Client::DeleteCharacterByNameFromCache(const std::string& character_name) {
	auto character_it = std::find_if(m_account_characters.begin(), m_account_characters.end(),
		[&character_name](const auto& character) {
			return character.name == character_name && character.deleted_at <= 0;
		});

	if (character_it == m_account_characters.end()) {
		LogCharacterSets("Character [{}] not found for account [{}]",
			character_name, GetAccountID());
		return false;
	}

	uint32 character_id = character_it->id;

	m_account_characters.erase(character_it);

	LogCharacterSets("Removed character [{}] (ID: {}) from cache for account [{}]",
		character_name, character_id, GetAccountID());
