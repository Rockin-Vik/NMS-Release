		...
		*/
		...
		if (GetPet()) {
			if (!GetPetByID(focused_pet_id)) {
				...
				// The GetPet() check above is not a safe guarantee that petids is
				// non-empty: GetPet() can drop entries itself, and ValidatePetList()
				// can empty the vector outright. Indexing [0] unchecked is an
				// out-of-bounds read on a live zone.
				if (!petids.empty()) {
		...
		// Coalesced pet-list flush -- one packet per loop however many pets changed.
		//
		// Gated on CLIENT_CONNECTED: during zone-in the client is still loading and
		// wipes its pet tracking when it detects the zone change, so a list sent
		// mid-load is applied and then destroyed. Holding the dirty flag until the
		// client is fully in the zone means the list always lands after the wipe.
		if (IsPetListDirty() && client_state == CLIENT_CONNECTED) {
			SendPetList();
		}
...
void Client::OnDisconnect(bool hard_disconnect) {
	if (hard_disconnect) {
		// NMS: no LeaveGroup() here anymore - crash-reconnect group grace. This fires on the
		// FIRST tick a dead connection is noticed, which used to sever the group instantly
		// (and fully disband a duo) before the player ever had a chance to reconnect. Group
		// removal now happens where each exit path already handles it loudly: camp_timer /
		// linkdead_timer expiry / ~Client(), all of which notify the remaining members. A
		// crasher who returns inside the LD window rejoins their group (world no longer
		// strips the group_id rows on a fresh login).
	...
	if (RuleB(Custom, MulticlassingEnabled)) {
		...
		for (int classID = 1; classID <= 16; ++classID) {
			if (classesBitmask & (1 << (classID - 1))) { // Check if the class is unlocked.
				uint8 req_level = spells[m->spell_id].classes[classID - 1];
				if (m->scribing == memSpellScribing) {
					// For scribing, allow any level as long as the class can eventually use the spell
					if (req_level < 255) {
						isEligibleForSpell = true;
						break;
					}
				} else {
					// For memorizing, enforce the level requirement
					if (GetLevel() >= req_level) {
						isEligibleForSpell = true;
						break;
					}
				}
		...
		if (!isEligibleForSpell) {
			if (m->scribing == memSpellScribing) {
				Client::Message(Chat::Red, "None of your unlocked classes can learn this spell.");
			} else {
				Client::Message(Chat::Red, "None of your classes are eligible to memorize this spell at your level.");
			}
			return;
		}
	} else {
		if (m->scribing != memSpellForget) {
			uint8 req_level = spells[m->spell_id].classes[GetClass() - 1];
			if (m->scribing == memSpellScribing) {
				if (!IsPlayerClass(GetClass()) || req_level >= 255) {
					Message(Chat::Red, "Your class cannot scribe this spell.");
					return;
				}
			} else {
				if (!IsPlayerClass(GetClass()) || GetLevel() < req_level) {
					MessageString(
						Chat::Red,
						SPELL_LEVEL_TO_LOW,
						std::to_string(req_level).c_str(),
...
void Client::OPGMTraining(const EQApplicationPacket *app)
{
	...
	if (!RuleB(Character, AllowCrossClassTrainers)) {
		if (GetClass() != trains_class) {
			Message(Chat::White, "You are not a member of the %s class guild.  Begone.", class_names[GetClass()].c_str());
			safe_delete(outapp);
			return;
		}
	}
	...
	for (int sk = EQ::skills::Skill1HBlunt; sk <= EQ::skills::HIGHEST_SKILL; ++sk) {
		if (!CanHaveSkill((EQ::skills::SkillType)sk)) {
			gmtrain->skills[sk] = 0;
			continue;
		}
