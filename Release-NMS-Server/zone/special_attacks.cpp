...
void Client::SendCombatAbilityTimers() {
	...
	if (ClientVersion() >= EQ::versions::ClientVersion::RoF2 && p_timers.Enabled(pTimerCombatAbility2)) {
		if (timers->strike == 0) {
			timers->strike = p_timers.GetRemainingTime(pTimerCombatAbility2);
		}
	}

	// Fallback if individual timers are not enabled but the shared combat ability timer is
	if (p_timers.Enabled(pTimerCombatAbility)) {
		uint32 combat_time = p_timers.GetRemainingTime(pTimerCombatAbility);
		if (timers->backstab == 0) timers->backstab = combat_time;
		if (timers->bash == 0) timers->bash = combat_time;
		if (timers->frenzy == 0) timers->frenzy = combat_time;
		if (timers->kick == 0) timers->kick = combat_time;
		if (timers->strike == 0 && ClientVersion() < EQ::versions::ClientVersion::RoF2) timers->strike = combat_time;
	}

	timers->autoskill_bitmask = 0;
	if (GetAutoSkillStatus(EQ::skills::SkillBackstab))   timers->autoskill_bitmask |= (1 << 0);
	if (GetAutoSkillStatus(EQ::skills::SkillBash))       timers->autoskill_bitmask |= (1 << 1);
	if (GetAutoSkillStatus(EQ::skills::SkillKick))       timers->autoskill_bitmask |= (1 << 2);
	if (GetAutoSkillStatus(EQ::skills::SkillFrenzy))     timers->autoskill_bitmask |= (1 << 3);
	if (GetAutoSkillStatus(EQ::skills::SkillTaunt))      timers->autoskill_bitmask |= (1 << 4);
	if (GetAutoSkillStatus(EQ::skills::SkillDragonPunch)) timers->autoskill_bitmask |= (1 << 5);
	if (GetAutoSkillStatus(EQ::skills::SkillEagleStrike)) timers->autoskill_bitmask |= (1 << 6);
	if (GetAutoSkillStatus(EQ::skills::SkillFlyingKick))  timers->autoskill_bitmask |= (1 << 7);
	if (GetAutoSkillStatus(EQ::skills::SkillRoundKick))   timers->autoskill_bitmask |= (1 << 8);
	if (GetAutoSkillStatus(EQ::skills::SkillTigerClaw))   timers->autoskill_bitmask |= (1 << 9);
...
{
	struct AutoSkillGuard {
		Client* m_client;
		bool m_active;
		AutoSkillGuard(Client* c, bool active) : m_client(c), m_active(active) {
			if (m_active) {
				m_client->SetEntityVariable("auto_skill_active", "1");
			}
		}
		~AutoSkillGuard() {
			if (m_active) {
				m_client->DeleteEntityVariable("auto_skill_active");
			}
		}
	};

	bool is_autoskill = EntityVariableExists("auto_skill");
	AutoSkillGuard guard(this, is_autoskill);
...
void Mob::ProjectileAnimation(Mob* to, int item_id, bool IsArrow, float speed, float angle, float tilt, float arc, const char *IDFile, EQ::skills::SkillType skillInUse, int item_type_override) {
	if (!to)
		return;
	...
	// NMS: allow callers (e.g. spell bolts) to force a specific projectile item_type.
	// The RoF2 client picks the launch animation from item_type -- Arrow(27) -> bow-shot,
	// weapon types -> a melee swing. A non-weapon type (e.g. ItemTypeSpell) yields no
	// attack animation, so the bolt just flies.
	if (item_type_override >= 0)
		item_type = (uint8)item_type_override;
...
{
	uint16 taunt_skill = GetSkill(EQ::skills::SkillTaunt);
	if (taunt_skill == 0 && IsClient() && CastToClient()->CanHaveSkill(EQ::skills::SkillTaunt)) {
		CastToClient()->SetSkill(EQ::skills::SkillTaunt, 1);
		taunt_skill = 1;
	}
	...
	if (!RuleB(Combat, ClassicTauntSystem)) {
		...
		if (GetLevel() >= 60 && level_difference < 0) {
			...
			taunt_chance = 50 * taunt_skill / (who->GetLevel() * 5 + 5);
			taunt_chance += level_difference * 5;
