...
bool Client::SwapItem(MoveItem_Struct* move_in) {
	...
	if (src_inst){
		if (
			...
			if (src_inst->GetID() % 1000000 == 999990) {
				Message(Chat::Red, "You cannot place Veeshan's Dimensional Pocket in the shared bank.");
				return false;
			}
		}
	...
	*/
	...
	if (move_in->number_in_stack > 0) {
		...
		// Swap succeeded! Fetch item now in dst_slot_id to attune it.
		EQ::ItemInstance* equipped_inst = m_inv.GetItem(dst_slot_id);
		if (equipped_inst && (dst_slot_id <= EQ::invslot::EQUIPMENT_END) && dst_slot_id >= EQ::invslot::EQUIPMENT_BEGIN) {
			if (equipped_inst->GetItem()->Attuneable) {
	...
	for(int book_index = 0; book_index < EQ::spells::SPELLBOOK_SIZE; book_index++) {
		...
		for (int effect_id = 0; effect_id < 12; effect_id++) {
			if ((spells[spell_id].base_value[effect_id] % 1000000) == (item_id % 1000000)) {
				if (spells[spell_id].effect_id[effect_id] == SE_SummonItem) {
					LogDebug("CanSummonItem: Spell [{}] can summon item [{}]", spell_id, item_id);
					return true;
				}

				if (spells[spell_id].effect_id[effect_id] == SE_SummonItemIntoBag) {
...
int64_t Client::GetStatEntryValue(StatEntry label)
{
	...
	switch (label)
	{
		...
		case statHealAmt:
			return itembonuses.HealAmt + spellbonuses.HealAmt + aabonuses.HealAmt;
		...
		case statDoTCritRatio: {
			int value = GetSharedDotCritDmgIncrease();
		...
		case statMeleeCritRatio: {
			return GetCritDmgMod(EQ::skills::HIGHEST_SKILL);
		}
		...
		case statCapDSMitigation:
			return RuleI(Character, ItemDSMitigationCap);
...
)
{
	...
	SummonApocItem(
		...
		aug6,
		is_attuned,
		slot_id == INVALID_INDEX ? EQ::invslot::slotCursor : slot_id
	);
}
