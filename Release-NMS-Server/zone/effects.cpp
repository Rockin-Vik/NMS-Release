	...
	if (chance > 0 || (IsOfClientBot() && (HasClass(Class::Wizard)) && GetLevel() >= RuleI(Spells, WizCritLevel))) {
		...
		if (Critical) {
			...
			entity_list.FilteredMessageClose(
				...
				Chat::SpellCrit,
				FilterSpellCrits,
				fmt::format("{} delivers a critical blast! ({}) ({})", GetCleanName(), -value, spells[spell_id].name).c_str()
			);

			if (IsClient()) {
				Message(Chat::SpellCrit, fmt::format("You deliver a critical blast! ({}) ({})", -value, spells[spell_id].name).c_str());
			}
	...
	if (RuleB(Custom, MulticlassingEnabled)) {
		...
		for (int class_index = 0; class_index < 16; ++class_index) {
			if (class_bit & (1 << class_index)) {
				...
				if (level_required != 255) { // QoL Level-Free Tome Scribing
					canLearn = true;
		...
		if (false && level_to_use > GetLevel()) { // QoL Level-Free Tome Scribing
...
bool Client::MemorizeSpellFromItem(uint32 item_id) {
	...
	// Check if the player's level is too low for any of the classes (Bypassed for QoL Level-Free Spell Scribing)
	if (false && too_low && level_to_use > GetLevel()) {
