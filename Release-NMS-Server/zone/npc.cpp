...
NPC::~NPC()
{
	...
	// Hand-in bucket. Freed via original_items, which owns every ItemInstance clone the NPC ever made
	// (items shares the same pointers - shallow copy - so freeing both would double-free).
	//
	// The normal paths already free these: ResetHandin() on an ordinary NPC after each trade,
	// ResetMultiQuest() on a multi-quest NPC when the quest completes or when it dies and hands
	// everything back. This is the backstop for the path none of them cover - a multi-quest NPC that
	// leaves play WITHOUT dying (a repop, or a zone shutting down) - which otherwise leaked every item
	// still banked on it.
...
void NPC::DoPetCommand(int pet_command_id, Mob* target) {
	switch (pet_command_id) {
		...
		case PET_GUARDME:
			...
			break;
		case PET_SIT:
			// Toggle: sit unless already sitting. The old `!GetPetOrder() == SPO_Sit`
			// parsed as `(order == SPO_Follow)`, so the toggle on a GUARDING pet ran
			// the stand path and overwrote its order with the stale previous one --
			// the pet silently abandoned its guard post.
	...
	// Charmed chains can leave a pet owned by an NPC -- CastToClient() on that
	// owner is a bad cast, so only a real client gets the stats refresh.
...
void NPC::ConfigureInitialCommands() {
	...
	if (!owner && GetSwarmOwner()) {
		...
		// Swarm pets read the Class::None settings bucket (the one /pet swarm
		// writes). Was a no-op `==` typo; swarm pets have no pet spell id so
		// GetPetOriginClass() already returned None -- this makes it explicit
...
void NPC::SetTaunting(bool is_taunting) {
	...
	// Only apply TauntTogglesPetTanking rules to pets/swarm pets of clients or bots.
	// NPC pets should never gain BeingAggroImmunity because that prevents other NPCs/pets from attacking them.
	bool is_client_bot_pet = (IsPet() && IsPetOwnerOfClientBot()) ||
	                         (GetSwarmInfo() && GetSwarmInfo()->GetOwner() && GetSwarmInfo()->GetOwner()->IsOfClientBot());

	if (is_client_bot_pet) {
...
)
{
	...
	if (!m_handin_started) {
		for (const auto &i: items) {
			...
			// Stamp the owner. On a multi-quest NPC this bucket is carried across trades (ResetHandin
			// deliberately does not clear it), so entries handed in by DIFFERENT players accumulate
			// side by side. Recording who each one came from is what lets the NPC give them back to
			// the right person if it is killed before the quest is completed.
			h.items.emplace_back(
				HandinEntry{
					...
					.handed_in_by = c ? c->CharacterID() : 0
				}
			);
		}
	}
	...
	// Multi-quest: mark which handed-in items are a contribution to the quest, so they are BANKED on
	// the NPC rather than handed straight back by ReturnHandinItems (which returns everything not
	// flagged here). Anything unflagged goes back to the player, so this predicate decides whether a
	// multi-quest can accumulate an item at all.
	//
	// It MUST use the same matching rules as the fulfilment check below, or an item can satisfy the
	// requirement while never being recognised as a contribution. It previously compared the raw id
	// string and demanded an exact count match, which broke two things silently:
	//
	//   - Tiered items. An Enchanted/Legendary drop is base_id + 1,000,000 / + 2,000,000. The
	//     fulfilment check normalises (id % 1000000), so a tiered item COMPLETES a quest - but the raw
	//     string compare never matched, so it could never be BANKED toward one. The player watched it
	//     bounce back to their cursor and assumed the NPC did not want it.
	//   - Partial stacks. "10 Bone Chips" with a player contributing 4 is the entire point of a
	//     multi-quest, and 4 != 10 meant it never stuck.
	//
	// Any quantity of a required item is a legitimate partial contribution, so id match alone is the
	// condition - count is the fulfilment check's business, not ours. Items that match NO requirement
	// stay unflagged and are still returned to the player, which is what keeps a mis-handed item safe.
	//
	// Iterate handin_items, not m_hand_in.items: on the first CheckHandin call m_hand_in has not been
	// populated yet (that happens further down), so flagging it there was a no-op on the first pass.
	// handin_items aliases h.items on the first call and m_hand_in.items thereafter, and h is copied
	// into m_hand_in below, so the flags survive either way.
	//
	// This runs BEFORE before_handin_state is snapshotted, so the flags are captured in that snapshot
	// and survive the restore that happens when the requirement is not yet met - which is exactly the
	// path a partial multi-quest contribution takes.
	if (IsMultiQuestEnabled()) {
		for (auto &h_item: handin_items) {
			for (const auto &r_item: r.items) {
				const bool id_match =
					(!normalize && h_item.item_id == r_item.item_id) ||
	...
	if (!m_handin_started) {
		...
		if (IsMultiQuestEnabled()) {
			// original_items is the OWNERSHIP list: every ItemInstance clone this NPC has made is freed
			// through it (ResetMultiQuest / ResetHandin / ~NPC all free via original_items, and items
			// shares the same pointers - it is a shallow copy).
			//
			// So it must never forget a clone. Overwriting it with `items` would do exactly that on a
			// multi-quest NPC: ReturnHandinItems erases the items it hands back from `items` but leaves
			// them in original_items, and this runs again on the NEXT hand-in - so a straight
			// assignment drops those pointers on the floor and nothing ever frees them. One leaked
			// clone per item a player hands a multi-quest NPC that the quest does not want.
			//
			// Append only what is new instead, so original_items keeps owning everything it ever made.
			// (h carries the previous bucket forward - see `h = m_hand_in` at the top of this function -
			// so m_hand_in.original_items still holds the earlier trades' entries at this point.)
			for (const auto &i : m_hand_in.items) {
				const bool already_owned = std::any_of(
					m_hand_in.original_items.begin(),
					m_hand_in.original_items.end(),
					[&](const HandinEntry &o) { return o.item == i.item; }
				);

				if (!already_owned) {
					m_hand_in.original_items.push_back(i);
				}
			}
		}
		else {
			// Non-multi-quest: ResetHandin() frees original_items and wipes the bucket after every
			// trade, so there is nothing to carry forward and a straight snapshot is correct.
...
// The next free parcel slot for a character, without needing a Client object.
//
// Client::FindNextFreeParcelSlot does the same thing, but it is a member function and an NPC can die
// with no client anywhere near it - and the owner we are returning an item to is, by definition,
// often not online. Same query, same rules.
static int32 NMSFindFreeParcelSlot(uint32 char_id)
{
	const auto parcels = CharacterParcelsRepository::GetWhere(
		database,
	...
	for (uint32 i = PARCEL_BEGIN_SLOT; i <= (uint32) RuleI(Parcel, ParcelMaxItems); i++) {
		const auto it = std::find_if(
			parcels.cbegin(), parcels.cend(),
			[&](const auto &p) { return p.slot_id == i; }
		);
		if (it == parcels.cend()) {
			return i;
		}
...
// The NPC died still holding multi-quest hand-in items that nobody had completed the quest with.
// Give each one back to the player who handed it in.
//
// This used to put them on the CORPSE (AddLootDrop with a NewNpcEntity lootdrop entry). That is the
// wrong destination: these are not loot. They are one specific player's property, temporarily in the
// NPC's hands, and handing them to whoever happened to land the killing blow is no more correct than
// handing them to everybody. It also dropped any NoDrop item entirely, destroying it.
//
// So they never touch the loot system at all now. They go straight back to their owner, the same way
// a quest turn-in reward does.
void NPC::ReturnMultiQuestItemsOnDeath()
{
	if (!IsMultiQuestEnabled()) {
		return;
	}
	...
	for (auto &i : m_hand_in.items) {
		if (!i.is_multiquest_item || !i.item) {
			continue;
		}

		if (i.handed_in_by == 0) {
			// Cannot happen: every entry is stamped in CheckHandin. Log loudly rather than quietly
			// destroy somebody's item if it somehow does.
			LogError(
				"NPC [{}] holds multi-quest item [{}] with no recorded owner - cannot return it",
				GetCleanName(), i.item_id
			);
			continue;
		}

		// Owner is standing here: straight to their cursor, exactly like a turn-in reward. Pushing the
		// ItemInstance itself (rather than re-creating it from an id) keeps its charges, augments,
		// attunement, bag contents and custom data intact.
		//
		// The return value is NOT "did the push work". PushItemOnCursor always puts the item on the
		// cursor and always tells the client about it; what it returns is whether the follow-up
		// database.SaveCursor() persisted. So a false here means the player HAS the item - treating it
		// as a failure and parcelling it as well would hand them two copies of it.
		if (Client *owner = entity_list.GetClientByCharID(i.handed_in_by)) {
			if (!owner->PushItemOnCursor(*i.item, true)) {
				LogError(
					"NPC [{}] returned multi-quest item [{}] to [{}] but failed to persist their cursor",
					GetCleanName(), i.item_id, owner->GetCleanName()
				);
			}
			owner->Message(
				Chat::Yellow,
				"%s died before the task was finished. Your %s has been returned to you.",
				GetCleanName(), i.item->GetItem()->Name
			);
			LogNpcHandin(
				"[{}] returned multi-quest item [{}] to [{}] on death",
				GetCleanName(), i.item_id, owner->GetCleanName()
			);
			continue;
		}

		// Owner is not here. Parcel it - the item is theirs, and there is nowhere in this zone it
		// could sit that would ever reach them.
		//
		// This mirrors Client::DoParcelSend (parcels.cpp) exactly, and it has to: parcel retrieval
		// rebuilds the item from ONLY what is written here, via CreateItem(item_id, quantity, aug1..6)
		// plus the containers table. Anything not stored is destroyed on retrieval - so the augments,
		// the real charge count and any bag contents all have to be written out explicitly.
		const int32 slot = NMSFindFreeParcelSlot(i.handed_in_by);
		if (slot == INVALID_INDEX) {
			LogError(
				"NPC [{}] could not return multi-quest item [{}] to char [{}] - their parcel box is "
				"full. The item is LOST.",
				GetCleanName(), i.item_id, i.handed_in_by
			);
			continue;
		}
		...
		p.id        = 0;
		p.char_id   = i.handed_in_by;
		p.from_name = GetCleanName();
		p.note      = "Returned - I was slain before your task was complete.";
		p.slot_id   = slot;
		p.sent_date = time(nullptr);
		p.item_id   = i.item->GetID();
		// Charges, not "1": a non-stackable with charges (a clicky, a limited-use item) carries them
		// here, and writing 1 would quietly strip the rest away.
		p.quantity  = i.item->IsStackable()
			? std::max(i.item->GetCharges(), static_cast<int16>(1))
			: (i.item->GetCharges() >= 0 ? i.item->GetCharges() : 1);

		if (i.item->IsAugmented()) {
			const auto augs = i.item->GetAugmentIDs();
			p.aug_slot_1 = augs.at(0);
			p.aug_slot_2 = augs.at(1);
			p.aug_slot_3 = augs.at(2);
			p.aug_slot_4 = augs.at(3);
			p.aug_slot_5 = augs.at(4);
			p.aug_slot_6 = augs.at(5);
		}

		const auto inserted = CharacterParcelsRepository::InsertOne(database, p);
		if (inserted.id == 0) {
			LogError(
				"NPC [{}] FAILED to parcel multi-quest item [{}] back to char [{}] - the item is LOST",
				GetCleanName(), i.item_id, i.handed_in_by
			);
			continue;
		}

		// A handed-in bag keeps what is inside it. These rows are the only record of the contents;
		// without them the owner gets an empty bag back.
		if (i.item->IsNoneEmptyContainer()) {
			std::vector<CharacterParcelsContainersRepository::CharacterParcelsContainers> contents;
			for (const auto &kv : *i.item->GetContents()) {
				if (!kv.second) {
					continue;
				}
				CharacterParcelsContainersRepository::CharacterParcelsContainers cpc{};
				cpc.parcels_id = inserted.id;
				...
				if (kv.second->IsAugmented()) {
					const auto augs = kv.second->GetAugmentIDs();
				...
				contents.push_back(cpc);
			}
			if (!contents.empty()) {
				CharacterParcelsContainersRepository::InsertMany(database, contents);
			}
		}

		// Tell them it arrived. Writing the row is not enough on its own: the parcel window is populated
		// from a cached list the client is sent, so an owner who is online in ANOTHER zone would sit
		// there with no notification and an unchanged parcel box until they next zoned.
		//
		// This is the same notification Client::DoParcelSend fires (parcels.cpp:531). World broadcasts
		// ServerOP_ParcelDelivery to every booted zone; whichever one holds a client of this name shows
		// the "parcel has arrived" message and refreshes their parcel status. If they are offline it
		// simply matches nobody, which is correct - the row is already safely in the table and they will
		// see it on their next login. We cannot use Client::SendParcelDeliveryToWorld here because there
		// is, by definition, no Client object for an absent owner - but it only wraps this packet.
		const std::string owner_name = database.GetCharNameByID(i.handed_in_by);
		if (!owner_name.empty()) {
			auto out = std::make_unique<ServerPacket>(ServerOP_ParcelDelivery, sizeof(Parcel_Struct));
			auto pd  = (Parcel_Struct *) out->pBuffer;
			pd->item_slot = inserted.slot_id;
			strn0cpy(pd->send_to, owner_name.c_str(), sizeof(pd->send_to));
			worldserver.SendPacket(out.get());
		}
		else {
			LogError(
				"NPC [{}] parcelled multi-quest item [{}] to char [{}] but could not resolve their name "
				"to notify them - the item is safe, they will see it on next login",
				GetCleanName(), i.item_id, i.handed_in_by
			);
		}

		LogNpcHandin(
			"[{}] parcelled multi-quest item [{}] back to absent char [{}] on death",
			GetCleanName(), i.item_id, i.handed_in_by
		);
	}

	// Every item has now been handed back (cursor or parcel), so the bucket's ItemInstance clones are
	// nobody's property any more. Nothing else frees them on this path: ResetHandin deliberately skips
	// multi-quest NPCs, ResetMultiQuest only runs when a quest is COMPLETED, and ~NPC() does not touch
	// m_hand_in - so without this, every item ever handed to a multi-quest NPC leaks when it dies.
	//
	// ResetMultiQuest is the right tool: items and original_items share the same ItemInstance pointers
	// (original_items is a shallow vector copy), and it frees via original_items and clears both. Safe
	// to call here because Death() runs once and the NPC is torn down straight after - there is no
	// later path that could touch a freed pointer, and no double-free.
	ResetMultiQuest();
}
