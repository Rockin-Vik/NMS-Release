...
bool Perl_Mob_SpellOnTarget(Mob* self, uint16 spell_id, Mob* target) // @categories Spells and Disciplines
{
	if (self) {
...
{
	// First pet (index 0) -- kept exactly as it always was so existing quests are
	// unaffected. For multiclass multi-pet awareness use GetActivePet /
	// GetAllPets / GetPetByIndex / GetPetCount below.
	return self->GetPet();
}

Mob* Perl_Mob_GetActivePet(Mob* self) // @categories Script Utility, Pet
{
	// The pet currently focused in the player's pet window
	return self->GetActivePet();
}

Mob* Perl_Mob_GetPetByIndex(Mob* self, int idx) // @categories Script Utility, Pet
{
	if (idx < 0 || idx >= 255) {
		return nullptr;
	}

	return self->GetPet(static_cast<uint8>(idx));
}

int Perl_Mob_GetPetCount(Mob* self) // @categories Script Utility, Pet
{
	return static_cast<int>(self->GetAllPets().size());
}

perl::array Perl_Mob_GetAllPets(Mob* self) // @categories Script Utility, Pet
{
	perl::array result;
	for (Mob* pet : self->GetAllPets())
	{
...
void perl_register_mob()
{
	...
	package.add("GetActivePet", &Perl_Mob_GetActivePet);
	package.add("GetAllPets", &Perl_Mob_GetAllPets);
	package.add("GetPet", &Perl_Mob_GetPet);
	package.add("GetPetByIndex", &Perl_Mob_GetPetByIndex);
	package.add("GetPetCount", &Perl_Mob_GetPetCount);
	...
	package.add("SpellOnTarget", (bool(*)(Mob*, uint16, Mob*))&Perl_Mob_SpellOnTarget);
