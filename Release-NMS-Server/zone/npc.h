...
public:
	...
	struct HandinEntry {
		...
		// Character who handed THIS item in.
		//
		// A multi-quest bucket accumulates items from several different players - that is the whole
		// point of multi-quest - so "the items in this NPC" have no single owner. If the NPC is killed
		// before anyone completes the quest, each player must get their OWN items back. Without this
		// the server had no idea who anything belonged to, and the items were handed to the loot
		// system, which gave every eligible looter a COPY of all of them.
		uint32           handed_in_by       = 0;
	};
	...
	// The NPC was killed while still holding multi-quest hand-in items nobody had completed the quest
	// with. Give each item back to the player who handed it in - to their cursor if they are here, by
	// parcel if they are not. See the definition in npc.cpp for why they must not go on the corpse.
	void ReturnMultiQuestItemsOnDeath();
