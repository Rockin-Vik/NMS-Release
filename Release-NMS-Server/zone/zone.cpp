...
void Zone::LoadTempMerchantData()
{
	...
	for (const auto& id_str : npc_ids) {
		uint32_t npc_id = Strings::ToUnsignedInt(id_str);
		tmpmerchanttable[npc_id].clear();
	}
...
void Zone::ApplyGlobalBuffs()
{
	...
	for (auto &e: entity_list.GetClientList()) {
		int timer = (int) Mob::RandomTimer(1000, 10000);
...
{
	static const uint32 PERM_BUFF_IDS[] = {43002, 43005, 36856, 17779};
	static const int    PERM_BUFF_COUNT  = 4;

	if (RuleB(Custom, PermanentServerBuffsEnabled)) {
		// Set duration to now + 1 year (refreshed every rule reload / server start)
		const uint32 one_year = static_cast<uint32>(time(nullptr)) + 31536000;

		for (int i = 0; i < PERM_BUFF_COUNT; i++) {
			auto buff     = GlobalBuffsRepository::NewEntity();
			buff.spell_id = static_cast<int>(PERM_BUFF_IDS[i]);
			buff.duration = static_cast<int>(one_year);
			GlobalBuffsRepository::ReplaceOne(database, buff);
		}

		LogInfo("NMS: Permanent server buffs ENABLED - 1-year duration set for spells [43002, 43005, 36856, 17779]");

		// Reload cache and notify players buffs are incoming
		ApplyGlobalBuffs();
	}
	else {
		database.QueryDatabase(
			"DELETE FROM global_buffs WHERE spell_id IN (43002, 43005, 36856, 17779)"
		);

		LogInfo("NMS: Permanent server buffs DISABLED - entries removed");

		// Silently reload cache — no tingling message
		database.LoadGlobalBuffs();

		// Strip the buff effects from all active players in this zone
		for (auto& e : entity_list.GetClientList()) {
			for (int i = 0; i < PERM_BUFF_COUNT; i++) {
				e.second->BuffFadeBySpellID(PERM_BUFF_IDS[i]);
			}
		}
	}
}
