...
public:
	...
	int CalculateWeaponBackstab(EQ::ItemInstance* wpn, Mob* other);
	int GetWeaponBackstabDamage(EQ::ItemInstance* inst, Mob *target);
	...
	void ProjectileAnimation(Mob* to, int item_id, bool IsArrow = false, float speed = 0, float angle = 0, float tilt = 0, float arc = 0, const char *IDFile = nullptr, EQ::skills::SkillType skillInUse = EQ::skills::SkillArchery, int item_type_override = -1);
	...
	int64 CalcFocusEffect(focusType type, uint16 focus_id, uint16 spell_id, bool best_focus=false, uint16 casterid = 0, Mob *caster = nullptr, bool is_buff_tic = false);
	...
	void TrySympatheticProc(Mob *target, uint32 spell_id, bool is_buff_tic = false);
	uint16 GetSympatheticFocusEffect(focusType type, uint16 spell_id, bool is_buff_tic = false);
	bool TryFadeEffect(int slot);
	void DispelMagic(Mob* casterm, uint16 spell_id, int effect_value, bool detrimental_only = false, bool benficial_only = false);
	...
	// Pet-list change tracking. Every mutation of petids flags this; Client::Process
	// flushes it once per loop as a single OP_PetList packet, so a multi-pet change
	// (a resummon, a wipe on death) costs one packet instead of one per pet.
	void MarkPetListDirty() { m_pet_list_dirty = true; }
	bool IsPetListDirty() const { return m_pet_list_dirty; }
	void ClearPetListDirty() { m_pet_list_dirty = false; }
	...
	int64 CalcAAFocus(focusType type, const AA::Rank &rank, uint16 spell_id, bool is_buff_tic = false);
