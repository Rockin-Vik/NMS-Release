	...
	if (!dead) {
		...
		for (Mob* mob : pets) {
			if (mob)
			{
				...
				if (pet && pet->GetPetSpellID() && !pet->IsCharmed()) {
...
void Client::FixModel(Spawn_Struct* npc) {
	...
	switch (npc->race) {
		...
		case Race::Orc: {
			...
			bool isElite = Strings::ContainsLower(npc->name, "centurion") ||
				Strings::ContainsLower(npc->name, "legionnaire");
			bool isStandard = Strings::ContainsLower(npc->name, "pawn");
...
void Client::ReadBook(BookRequest_Struct* book)
{
	...
	// GetItem() returns null whenever item_id is not a real item -- including the -1
	// default we get for a non-numeric txtfile -- and the old code dereferenced it
	// unconditionally. Nothing to tag if the item does not exist, so skip the block.
	const auto item_data = item_id > 0 ? database.GetItem(item_id) : nullptr;

	if (book->type == BookType::ItemInfo && RuleB(Custom, UseDynamicItemDiscoveryTags) && item_data) {
	...
	if (!b.text.empty()) {
		...
		// Item-info tooltips are server-generated text ("Discovered By: ..."), not a book
		// written in some language, so they must never be garbled. Test book->type, not
		// t->type: the SoF+ branch above overwrites t->type with the item's Book field.
		if (
			book->type != BookType::ItemInfo &&
			EQ::ValueWithin(b.language, Language::CommonTongue, Language::Unknown27)
		) {
...
void Client::SetAutoSkillStatus(EQ::skills::SkillType skill_id, bool enabled)
{
    ...
    // Sync timers and autoskill bitmask to client immediately
...
void Client::LinkDead()
{
	if (GetGroup())
	{
		...
		// NMS: membership is kept through the LD window (crash-reconnect grace) - no
		// DelMember here. If they reconnect in time, the zone-entry rejoin swaps their
		// session back into the group; if not, linkdead_timer expiry in Client::Process
		// runs LeaveGroup() and removes them (disbanding a duo) with proper notification.
	}
	Raid *raid = entity_list.GetRaidByClient(this);
	if(raid){
...
void Client::SuspendMinion(int value)
{
	...
	// One AA button, three context-sensitive actions (design confirmed 2026-07-18):
	//   1. Targeting one of your own live pets -> suspend exactly that pet.
	//   2. Otherwise, if any pets are stored   -> redeploy ALL stored pets.
	//   3. Otherwise                           -> suspend the focused pet
	//      (the pre-multiclass behavior, so untargeted use feels unchanged).
	// Storage is a vector persisted as DB pet ids 100+, so pets can be suspended
	// one at a time: suspend pets 1 and 2, keep pet 3 out, then one untargeted
	// click brings 1 and 2 back together.

	auto store_minion = [&](NPC* pet) {
		if (pet->IsCharmed()) {
			MessageString(Chat::SpellFailure, ONLY_SUMMONED_PETS);
			return;
		}

		if (!pet->GetPetSpellID()) {
			return;
		}
		...
		// Keeps the DB ids within 100..100+cap-1
		if (m_suspendedminions.size() >= RuleI(Custom, AbsolutePetLimit)) {
			Message(Chat::SpellFailure, "You cannot suspend any additional pets.");
			return;
		}

		PetInfo info;
		memset(&info, 0, sizeof(PetInfo));

		info.SpellID  = pet->GetPetSpellID();
		info.HP       = pet->GetHP();
		info.Mana     = pet->GetMana();
		info.petpower = pet->GetPetPower();
		info.size     = pet->GetSize();
		info.taunting = pet->IsTaunting();

		if (value >= 1) {
			pet->GetPetState(info.Buffs, info.Items, info.Name);
		} else {
			strn0cpy(info.Name, pet->GetName(), sizeof(info.Name));
		}

		m_suspendedminions.push_back(info);
	...
	// Redeploys every stored pet it can. Anything that cannot come out right now
	// (pet cap, class conflict, spawn failure) STAYS stored -- a suspended pet is
	// never lost; at worst it waits for the next attempt.
	auto deploy_all = [&]() {
		std::vector<PetInfo> still_stored;

		for (auto& info : m_suspendedminions) {
			if (!IsValidSpell(info.SpellID)) {
				continue;  // Empty/corrupt slot -- nothing real to lose, drop it
			}

			// IsPetAllowed() also messages the player about the specific conflict
			if (GetAllPets().size() >= RuleI(Custom, AbsolutePetLimit) ||
				!IsPetAllowed(info.SpellID)) {
				Message(Chat::SpellFailure, "%s remains suspended.", info.Name);
				still_stored.push_back(info);
				continue;
			}

			// Identify the created pet by watching petids grow -- same pattern as
			// zone-in recreation; never assume an index.
			const size_t pets_before = petids.size();

			MakePoweredPet(info.SpellID, spells[info.SpellID].teleport_zone,
				info.petpower, info.Name, info.size);
			ValidatePetList();

			NPC* pet = nullptr;
			if (petids.size() > pets_before) {
				auto pet_mob = GetPetByID(petids.back());
				if (pet_mob) {
			...
			pet->SetPetState(info.Buffs, info.Items);
			pet->SendPetBuffsToClient();
			pet->CalcBonuses();
			pet->SetHP(info.HP);
			pet->SetMana(info.Mana);
			pet->SetTaunting(info.taunting);
		...
		m_suspendedminions = still_stored;
	};

	// Branch 1: explicit target -- suspend that pet
	if (GetTarget() && IsMyPet(GetTarget()) && GetTarget()->IsNPC()) {
		store_minion(GetTarget()->CastToNPC());
		return;
	}

	// Branch 2: stored pets waiting -- bring them all back
	if (!m_suspendedminions.empty()) {
		deploy_all();
		return;
	}

	// Branch 3: nothing targeted, nothing stored -- suspend the focused pet
	auto focused = GetActivePet();
	if (focused && focused->IsNPC()) {
		store_minion(focused->CastToNPC());
	}
}
...
FACTION_VALUE Client::_GetFactionLevel(uint32 char_id, uint32 npc_id, uint32 player_race, uint32 player_class, uint32 player_deity, int32 pFaction, Mob* tnpc)
{
	...
	// few optimizations
	if (RuleB(Custom, MulticlassingEnabled) && HasClass(Class::Rogue) && sneaking && hidden) {
		return FACTION_INDIFFERENTLY;
	}
...
int Client::GetTrackingDistance() {
	...
	for (int i = Class::Warrior; i <= Class::Berserker; i++) {
		if (HasClass(i)) {
			uint16 multiplier = GetClassTrackingDistanceMultiplier(i);
			if (multiplier > 0) {
				distance = std::max(distance, static_cast<float>((base_skill + GetSkill(EQ::skills::SkillTracking)) * multiplier));
			} else if (base_skill > 0) {
				distance = std::max(distance, static_cast<float>(base_skill * 1));
			}
		}
	}
