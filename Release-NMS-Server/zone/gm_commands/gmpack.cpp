...
void command_gmpack(Client *c, const Seperator *sep)
{
	...
	if (rows.empty()) {
		c->Message(
			Chat::Yellow,
			"nms_gm_starter_pack is empty or missing - run the custom database migration (v32) first."
		);
		return;
	}
	...
	if (!box_item) {
		c->Message(
			Chat::Yellow,
			fmt::format(
				"GM Starter Box item [{}] is not in the item store - migrate v31 and rerun shared_memory.",
				box_row->item_id
			).c_str()
		);
		return;
