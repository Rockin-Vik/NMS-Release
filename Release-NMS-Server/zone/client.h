...
//	void TraderPriceUpdate(const EQApplicationPacket *app);
	...
	// Suspended pets, any number up to the pet cap. Persisted as DB pet ids 100+
	// (100 was the old single suspended slot, so existing saves load unchanged).
	inline std::vector<PetInfo>& GetSuspendedPetsInfo() { return m_suspendedminions; }
	...
	// flush is coalesced in Client::Process; call this directly only when the list
	// must land immediately (zone-in, after pets are recreated).
	void SendPetList();
...
private:
	...
	uint8  GetServerExpLevelCap() const;
	uint64 GetServerHardExpCap() const;
...
private:
	std::vector<NmsWaypointsRepository::NmsWaypoints> m_unlocked_waypoints = { };
	int m_expanded_waypoints = -1;
	int m_group_feature_state = -1;
	int m_auto_transport_state = -1;

public:
	std::vector<NmsWaypointsRepository::NmsWaypoints>& GetUnlockedWaypoints(bool force_reload = false);
	bool IsWaypointUnlocked(int32 waypoint_id);
	bool IsWaypointUnlocked(std::string waypoint_shortname);
	const NmsWaypointsRepository::NmsWaypoints* GetWaypoint(int waypoint_id);
	...
	bool BonusGrantsSkill(EQ::skills::SkillType skill_id) const;
...
private:
	...
	std::vector<PetInfo> m_suspendedminions; // stored (suspended) pets, in suspend order; DB pet ids 100+
...
public:
	...
	bool ShouldHideCorpse(Corpse* corpse) const;
	inline uint8 GetHideCorpseMode() const { return HideCorpseMode; }
	inline void SetHideCorpseMode(uint8 mode) { HideCorpseMode = mode; }
