...
int command_init(void)
{
	...
	if (
		...
		command_add("castspellnms", "[Spell ID or Spell Name] - Cast a spell from your spellbook out of combat", 5, command_castspellnms) ||
		...
		command_add("hero", "[show|set|setall] - View or set per-class experience", AccountStatus::GMAdmin, command_hero) ||
		...
		command_add("aaspike", "[show|timers|table|set|clear|sentinel|resend] - SPIKE: manual dynamic AA timer ids for the client spike (throwaway branch).", AccountStatus::GMMgmt, command_aaspike) ||
...
int command_realdispatch(Client *c, std::string message, bool ignore_status)
{
	...
	if (RuleB(Custom, GuideAudit) && c->Admin() >= 80) {
		std::string log_filename = fmt::format("logs/Guide{}Audit.log", c->GetName());
		FILE* f = fopen(log_filename.c_str(), "a");
		if (f) {
			std::string timestamp_str = string_from_time("%Y-%m-%d %H:%M:%S");
			std::string log_entry = fmt::format("[{}] {} (Acc: {}) used: {}\n", timestamp_str, c->GetName(), c->AccountName(), message);
			fprintf(f, "%s", log_entry.c_str());
			fclose(f);
		}
	}
