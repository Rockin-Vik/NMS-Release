...
Lua_Mob Lua_Mob::GetPet() {
	// First pet (index 0) -- unchanged so existing quests are unaffected. For
	// multi-pet awareness use GetActivePet / GetAllPets / GetPetByIndex /
	// GetPetCount.
	Lua_Safe_Call_Class(Lua_Mob);
	return Lua_Mob(self->GetPet());
}

Lua_Mob Lua_Mob::GetActivePet() {
	// The pet currently focused in the player's pet window
	Lua_Safe_Call_Class(Lua_Mob);
	return Lua_Mob(self->GetActivePet());
}

Lua_Mob Lua_Mob::GetPetByIndex(int idx) {
	Lua_Safe_Call_Class(Lua_Mob);
	if (idx < 0 || idx >= 255) {
		return Lua_Mob(nullptr);
	}
	return Lua_Mob(self->GetPet(static_cast<uint8>(idx)));
}

int Lua_Mob::GetPetCount() {
	Lua_Safe_Call_Int();
	return static_cast<int>(self->GetAllPets().size());
}

luabind::object Lua_Mob::GetAllPets(lua_State* L) {
...
luabind::scope lua_register_mob() {
	...
	.def("GetActivePet", &Lua_Mob::GetActivePet)
	.def("GetAllPets", &Lua_Mob::GetAllPets)
	.def("GetPet", &Lua_Mob::GetPet)
	.def("GetPetByIndex", &Lua_Mob::GetPetByIndex)
	.def("GetPetCount", &Lua_Mob::GetPetCount)
