...
void Client::Handle_OP_ZoneChange(const EQApplicationPacket *app) {
	...
	if (zoning_message == ZoningMessage::ZoneSuccess) {
		...
		LogDebug("Zoning [{}]: Rules prevent this char from zoning into [{}]", GetName(), target_zone_name);
...
{
	LogDebug("Zone [{}] is not available because target wasn't found or character insufficent level", zc->zoneID);
	cheat_manager.SetExemptStatus(Port, true);
	EQApplicationPacket *outapp = nullptr;
	outapp = new EQApplicationPacket(OP_ZoneChange, sizeof(ZoneChange_Struct));
	ZoneChange_Struct *zc2 = (ZoneChange_Struct*)outapp->pBuffer;
...
void Client::ZonePC(uint32 zoneID, uint32 instance_id, float x, float y, float z, float heading, uint8 ignorerestrictions, ZoneMode zm) {
	...
	if (ReadyToZone)
	{
		...
		if (IsLooting())
		{
			...
			if (entity == 0)
			{
				LogDebug("OP_EndLootRequest: Corpse with id of {} not found for {}.", entity_id_being_looted, GetCleanName());
			...
			{
				LogDebug("OP_EndLootRequest: Entity with id of {} was not corpse for {}.", entity_id_being_looted, GetCleanName());
				Corpse::SendLootReqErrorPacket(this);
			}
			else
			{
