...
void Client::CompleteConnect()
{
	...
	// The client is now fully in the zone -- (re)queue the pet list so the flush in
	// Client::Process delivers it past the client's zone-in tracking wipe. Redundant
	// when the zone-in path already flagged it (the flush coalesces), but this is
	// the belt-and-braces guarantee that every completed connect gets a list.
	MarkPetListDirty();
	...
	// Refresh appearance/buffs for EVERY pet, not just pet[0] -- with multiple pets
	// the others rendered stale equipment and buffs until some later event.
	for (Mob* pet : GetAllPets()) {
	...
	*/
	...
	for (int i = 0; i < MAX_PP_SKILL; i++) {
		if (i <= EQ::skills::HIGHEST_SKILL) {
			// Greyed whatever the value: a shelved class's skill keeps its number in the profile
			// (hero rule 5) and must not read as usable.
			if (!CanHaveSkill((EQ::skills::SkillType)i)) {
	...
	if (RuleB(NPC, PetZoneWithOwner)) {
		...
		for (int i = 0; i < m_petinfomulti.size(); i++) {
			...
			if (pet_info.SpellID > 1 && GetAllPets().size() < RuleI(Custom, AbsolutePetLimit) && pet_info.SpellID <= SPDAT_RECORDS) {
				// Create the pet using the stored data. Identify the created pet by
				// watching petids grow, NOT by assuming record i landed at petids[i]:
				// if any earlier record fails to spawn, the loop counter and the
				// vector index drift apart and every later pet gets the wrong saved
				// buffs/HP/equipment applied to it.
				const size_t pets_before = petids.size();
				...
				Mob* created = (petids.size() > pets_before) ? GetPetByID(petids.back()) : nullptr;
				if (created && created->IsNPC()) {
					NPC *pet = created->CastToNPC();
					if (pet)
					{
	...
	if (GetPet(0) && !petids.empty()) {
		focused_pet_id = petids[0];
		ConfigurePetWindow(GetPet(0));
	}

	// Queue the authoritative pet list now that the pets above have been recreated.
	// Deliberately NOT a direct SendPetList(): at this point the client is still
	// loading the zone, and its pet tracking wipes itself when it detects the zone
	// change -- a list sent now is applied and then destroyed. The dirty flag holds
	// the send until Client::Process sees CLIENT_CONNECTED, which is after the
	// client-side wipe has already happened. Unconditional on purpose: with no pets
	// the eventual send is count = 0, which clears stale gauges from the last zone.
	MarkPetListDirty();
...
void Client::Handle_OP_BlockedBuffs(const EQApplicationPacket *app)
{
	...
	if (bbs->Initialise == 1)
	{
		...
		if (!bbs->Pet) {
			for (int slot = 0; slot < GetMaxBuffSlots(); ++slot) {
				if (IsValidSpell(buffs[slot].spellid) && IsBlockedBuff(buffs[slot].spellid)) {
					BuffFadeBySlot(slot, true);
				}
			}
		} else {
			for (auto pet : GetAllPets()) {
				for (int slot = 0; slot < pet->GetMaxBuffSlots(); ++slot) {
					if (IsValidSpell(pet->GetBuffs()[slot].spellid) && IsBlockedPetBuff(pet->GetBuffs()[slot].spellid)) {
						pet->BuffFadeBySlot(slot, true);
					}
				}
			}
		}
...
void Client::Handle_OP_CastSpell(const EQApplicationPacket *app)
{
	...
	if (m_pp.mem_spells[castspell->slot] && m_pp.mem_spells[castspell->slot] == castspell->spell_id) {
		...
		if (RuleB(Custom, MulticlassingEnabled) && IsValidSpell(spell_to_cast) && !CanCastSpellAtLevel(spell_to_cast, GetLevel())) {
...
void Client::Handle_OP_GMNameChange(const EQApplicationPacket *app)
{
	...
	const bool used_name = database.IsNameUsedByPlayerOrBot(gmn->newname);
	if (!c) {
...
void Client::Handle_OP_GuildCreate(const EQApplicationPacket *app)
{
	...
	UpdateWho();
	guild_mgr.MemberAdd(new_guild_id, CharacterID(), GetLevel(), RuleB(Custom, MulticlassingEnabled) ? (GetClassesBits() + 1000) : GetClass(), GUILD_LEADER, GetZoneID(), GetName());
...
void Client::Handle_OP_GuildInviteAccept(const EQApplicationPacket *app)
{
	...
	guild_mgr.MemberAdd(guild_id, c_invitee->CharacterID(), c_invitee->GetLevel(), RuleB(Custom, MulticlassingEnabled) ? (c_invitee->GetClassesBits() + 1000) : c_invitee->GetClass(), response, c_invitee->GetZoneID(), std::string(c_invitee->GetCleanName()));
	SendGuildSpawnAppearance();
...
void Client::Handle_OP_Hide(const EQApplicationPacket *app)
{
	...
	if (RuleB(Custom, MulticlassingEnabled) && HasClass(Class::Rogue)) {
		hidechance = 100.0f;
	}
...
void Client::Handle_OP_ItemLinkClick(const EQApplicationPacket *app)
{
	...
	EQ::ItemInstance *inst =
		database.CreateItem(item, item->MaxCharges, ivrs->augments[0], ivrs->augments[1], ivrs->augments[2],
			ivrs->augments[3], ivrs->augments[4], 0, ivrs->augments[5] == 1);
	if (inst) {
		SendItemPacket(0, inst, ItemPacketViewLink);
		safe_delete(inst);
	}
...
void Client::Handle_OP_ItemVerifyRequest(const EQApplicationPacket *app)
{
	...
	// Fall back to selected target if target_id is invalid/0/self for detrimental spells
	if (spell_id > 0 && (target_id == 0 || (target_id == GetID() && IsDetrimentalSpell(spell_id)))) {
		auto selected_target = GetTarget();
		if (selected_target) {
			target_id = selected_target->GetID();
		}
	}
	...
	if (spell_id > 0 && (spells[spell_id].target_type == ST_Pet || spells[spell_id].target_type == ST_SummonedPet)) {
		auto active_pet = GetActivePet();
		target_id = active_pet ? active_pet->GetID() : 0;
	}
...
void Client::Handle_OP_ShopPlayerBuy(const EQApplicationPacket *app)
{
	...
	if (!item || (mp->playerid != GetID() && item->ID != mp->playerid)) {
		...
		if (tmp) entity_list.SendMerchantInventory(tmp, mp->itemslot, true);
		SendMerchantEnd();
		return;
	}

	if (CheckLoreConflict(item)) {
		MessageString(Chat::White, DUPE_LORE_MERCHANT, tmp ? tmp->GetCleanName() : "Merchant", item->Name);
		return;
	}
	...
	if (RuleB(Merchant, UsePriceMod) && tmp) {
	...
	if (inst && tmpmer_used && tmp) {
		...
		if (new_charges <= 0) {
			...
			if (tmp) entity_list.QueueClients(tmp, delitempacket); //que for anyone that could be using the merchant so they see the update
	...
	if (tmp && PlayerEventLogs::Instance()->IsEventEnabled(PlayerEvent::MERCHANT_PURCHASE)) {
		auto e = PlayerEvent::MerchantPurchaseEvent{
	...
	if (tmp && parse->PlayerHasQuestSub(EVENT_MERCHANT_BUY)) {
		const auto& export_string = fmt::format(
			"{} {} {} {} {}",
			tmp->GetNPCTypeID(),
...
void Client::Handle_OP_ShopPlayerSell(const EQApplicationPacket *app)
{
	...
	if (item->SummonedFlag) {
		if (vendor) {
			...
			Message(Chat::MerchantExchange, "That item has no value.");
		}
		return;
	}

	// An augmented item cannot be sold - refuse the sale rather than quietly eat the augments.
	//
	// merchantlist_temp has no augment columns (npcid, slot, zone_id, instance_id, itemid, charges), so
	// a merchant physically cannot stock an augmented item. The sale would take the item, pay for the
	// BASE item only, restock the merchant with a bare copy, and destroy every augment in it - including
	// the ornament, which lives in socket 6 - with nothing anywhere recording that they existed.
	//
	// Live EQ refuses this sale for the same reason. Telling the player to pull the augs first is the
	// only outcome that does not lose something they cannot get back.
	if (inst->IsAugmented()) {
		if (vendor) {
			Message(Chat::MerchantExchange, "%s tells you, 'Take the augments out of that first - I'll not have them destroyed.'", vendor->GetCleanName());
		} else {
			Message(Chat::MerchantExchange, "You must remove the augments from that item before it can be sold.");
		}
		return;
	}
	...
	if (RuleB(Merchant, UsePriceMod) && vendor) {
	...
	if (vendor && vendor->GetKeepsSoldItems()) {
		int freeslot = 0;
		if (
	...
	if (vendor && parse->PlayerHasQuestSub(EVENT_MERCHANT_SELL)) {
		const auto& export_string = fmt::format(
			"{} {} {} {} {}",
			vendor->GetNPCTypeID(),
			vendor->CastToNPC()->MerchantType,
	...
	if (vendor && PlayerEventLogs::Instance()->IsEventEnabled(PlayerEvent::MERCHANT_SELL)) {
		auto e = PlayerEvent::MerchantSellEvent{
	...
	mco->npcid = vendor ? vendor->GetID() : mp->npcid;
...
void Client::Handle_OP_Sneak(const EQApplicationPacket *app)
{
	...
	if (RuleB(Custom, MulticlassingEnabled)) {
		// Always send the sneak appearance packet so the client enters sneak mode
		// visually. Without this, Rogue multiclass characters never get the packet
		// and the client doesn't know sneak is active.
		auto outapp = new EQApplicationPacket(OP_SpawnAppearance, sizeof(SpawnAppearance_Struct));
		SpawnAppearance_Struct* sa_out = (SpawnAppearance_Struct*)outapp->pBuffer;
		sa_out->spawn_id = GetID();
		sa_out->type = 0x0F;
		...
		QueuePacket(outapp);
		safe_delete(outapp);
		// Send the success/fail chat message for Rogues
		if (HasClass(Class::Rogue)) {
			if (sneaking) {
...
void Client::Handle_OP_SpawnAppearance(const EQApplicationPacket *app)
{
	...
	else if (sa->type == AppearanceType::CancelSneakHide)
	{
		// Normal path: cancel both
		hidden          = false;
		improved_hidden = false;
		fake_hidden     = false;
		sneaking        = false;
...
void Client::Handle_OP_XTargetRequest(const EQApplicationPacket *app)
{
	...
	case MyPet:
	{
		...
		{
			XTargets[Slot].ID = 0;
			SendXTargetPacket(Slot, nullptr);
		}
		break;
	}
	case MyPetTarget:
