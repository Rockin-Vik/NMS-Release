...
//
// SummonApocItem has always taken aug6 - only the Perl bindings dropped it. That is the slot that
// matters: nothing in the game uses aug slot 5, while ~110k items use slot 6 (it is the ORNAMENTATION
...
{
	// slot_id was accepted and then silently dropped, so this landed on the cursor no matter what the
	// caller asked for. It is the only ReturnItem overload that takes a slot, so anything needing a
	// specific slot was pushed onto SummonFixedItem instead -- which rolls an item upgrade (see below).
...
void Perl_Client_ReturnItem(Client* self, uint32 item_id, int16 charges, bool attune, uint32 aug1, uint32 aug2, uint32 aug3, uint32 aug4, uint32 aug5, uint32 aug6, uint16 slot_id) // @categories Inventory and Items, Script Utility
{
	self->ReturnItem(item_id, charges, aug1, aug2, aug3, aug4, aug5, aug6, attune, slot_id);
}
...
void perl_register_client()
{
	...
	package.add("SummonFixedItem", (void(*)(Client*, uint32, int16, bool, uint32, uint32, uint32, uint32, uint32, uint32, uint16))&Perl_Client_SummonFixedItem);
	...
	package.add("ReturnItem", (void(*)(Client*, uint32, int16, bool, uint32, uint32, uint32, uint32, uint32, uint32, uint16))&Perl_Client_ReturnItem);
