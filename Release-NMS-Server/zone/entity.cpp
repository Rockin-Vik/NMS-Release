	...
	if (tsFirstSpawnOnQueue != 0xFFFFFFFF && (Timer::GetCurrentTime() - tsFirstSpawnOnQueue) > 50) {
		...
		while(iterator.MoreElements()) {
			...
			if (it == npc_list.end()) {
				...
				LogError("Error in EntityList::CheckSpawnQueue: Unable to find NPC [{}] for spawnId [{}]", ns->spawn.name, ns->spawn.spawnId);
			}
			else {
				NPC *pnpc = it->second;
...
void EntityList::SendZoneCorpses(Client *client)
{
	...
	for (auto it = corpse_list.begin(); it != corpse_list.end(); ++it) {
		...
		if (ent && !client->ShouldHideCorpse(ent)) {
			app = new EQApplicationPacket;
...
void EntityList::SendZoneCorpsesBulk(Client *client)
{
	...
	for (auto it = corpse_list.begin(); it != corpse_list.end(); ++it) {
		spawn = it->second;
		if (spawn && spawn->InZone() && !client->ShouldHideCorpse(spawn)) {
			memset(&ns, 0, sizeof(NewSpawn_Struct));
			spawn->FillSpawnStruct(&ns, client);
			bzsp->AddSpawn(&ns);
		}
...
void EntityList::HideCorpses(Client *c, uint8 CurrentMode, uint8 NewMode)
{
	...
	c->SetHideCorpseMode(NewMode);

	auto it = corpse_list.begin();
	while (it != corpse_list.end()) {
		Corpse *b = it->second;
		if (b) {
			c->SetHideCorpseMode(CurrentMode);
			bool was_hidden = c->ShouldHideCorpse(b);

			c->SetHideCorpseMode(NewMode);
			bool is_hidden = c->ShouldHideCorpse(b);
