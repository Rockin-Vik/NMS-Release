...
void ZoneDatabase::SaveBuffs(Client *client)
{
	...
	if (client->nobuff_invisible) {
		e.character_id   = client->CharacterID();
		e.slot_id        = 100;
		e.spell_id       = 5243; // Fading Memories placeholder
		e.caster_level   = client->GetLevel();
		e.caster_name    = client->GetName();
		e.ticsremaining  = -1;
		e.counters       = 0;
		e.numhits        = 0;
		e.melee_rune     = 0;
...
void ZoneDatabase::LoadBuffs(Client *client)
{
	...
	for (const auto& e : l) {
		if (e.slot_id == 100 && e.spell_id == 5243) {
			client->SetInvisible(Invisibility::Invisible);
			continue;
		}
...
void ZoneDatabase::SavePetInfo(Client *client)
{
    ...
    // Save suspended pets as DB pet ids 100, 101, ... (100 was the old single
    // suspended slot, so pre-existing saves load unchanged). No schema change:
    // `pet` is a plain int key. Empty slots are skipped rather than written as
    // spell_id=0 garbage rows.
    auto& suspended_pets = client->GetSuspendedPetsInfo();
    for (size_t i = 0; i < suspended_pets.size(); ++i) {
        if (suspended_pets[i].SpellID > 0) {
            save_pet_info(suspended_pets[i], 100 + i);
        }
    }
...
void ZoneDatabase::LoadPetInfo(Client *client)
{
    ...
    auto& suspended = client->GetSuspendedPetsInfo();

    // Clear the vectors to remove dangling pointers
    pets_info.clear();
    suspended.clear();
    ...
    for (const auto& e : info) {
        ...
        if (e.pet >= 100) {
            // Suspended pet (DB ids 100+). ORDER BY pet ASC plus contiguous ids at
            // save time means position in the vector == e.pet - 100, which the
            // buff/item loops below rely on.
    ...
    // NOT gated on !pets_info.empty(): a player can zone with suspended pets and no
    // live ones, and the old gate silently skipped restoring the suspended pets'
    // buffs and equipment in exactly that case (they redeployed naked).
    if (!buffs.empty())
	{
    for (const auto& e : buffs)
		{
			if (e.pet >= 100)
			{
				const size_t si = e.pet - 100;
				if (si >= suspended.size()) {
					continue;
				}
				...
				auto& sbuff = suspended[si].Buffs[e.slot];

				strn0cpy(sbuff.caster_name, e.castername.c_str(), sizeof(sbuff.caster_name));

				auto caster = entity_list.GetClientByName(sbuff.caster_name);

				sbuff.spellid = e.spell_id;
				sbuff.level = e.caster_level;
				sbuff.player_id = caster ? caster->GetID() : 0;
				sbuff.effect_type = BuffEffectType::Buff;
				sbuff.duration = e.ticsremaining;
				sbuff.counters = e.counters;
				sbuff.bard_modifier = e.instrument_mod;
			}
    ...
    // Same fix as the buff loop above: no !pets_info.empty() gate, or suspended
    // pets lose their equipment when the player zones with no live pets.
    if (!inventory.empty()) {
     ...
     if (!EQ::ValueWithin(e.slot, EQ::invslot::EQUIPMENT_BEGIN, EQ::invslot::EQUIPMENT_END)) {
       continue;
			...
			if (e.pet >= 100)
			{
				const size_t si = e.pet - 100;
				if (si < suspended.size()) {
					suspended[si].Items[e.slot] = e.item_id;
				}
			}
			else if(e.pet < pets_info.size())
			{
