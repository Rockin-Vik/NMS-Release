					...
					int16 resist_adjust, uint32 aa_id)
{
	...
	if (slot != CastingSlot::Discipline) {
		SendBeginCast(spell_id, cast_time);
	}
	...
	if (slot <= CastingSlot::MaxGems && slot >= CastingSlot::Gem1 || !IsClient()) {
		if (target && !target->IsMezzed()) {
			if(IsOfClientBotMerc() && IsEffectInSpell(spell_id, SE_CurrentHP)) {
				TrySympatheticProc(target, spell_id, false);
			}
			if (spell_success) {
...
// single target spells
		...
		case ST_SummonedPet:
		{
			if (!spell_target || !GetPetByID(spell_target->GetID())) {
				spell_target = GetActivePet();
			}

			if(!spell_target ||
				(spell_target->GetBodyType() != BodyType::Summoned &&
				 spell_target->GetBodyType() != BodyType::Summoned2 &&
				 spell_target->GetBodyType() != BodyType::Summoned3 &&
				 spell_target->GetBodyType() != BodyType::Animal &&
				 spell_target->GetBodyType() != BodyType::SummonedUndead))
			{
				LogSpells("Spell [{}] canceled: invalid target of body type [{}] (summoned pet)",
							spell_id, spell_target ? spell_target->GetBodyType() : BodyType::Humanoid);
		...
		case SingleTarget:
		{
			...
			if (spells[spell_id].target_type == ST_Pet || spells[spell_id].target_type == ST_SummonedPet) {
				if (spell_target && GetTarget() == spell_target && IsEffectInSpell(spell_id, SE_Illusion) && IsMyPet(spell_target)) {
					...
					// INTENTIONAL (confirmed 2026-07-18): pet-target spells hit ALL of the
					// caster's pets, not just the focused/targeted one -- one pet heal or
					// pet buff covers the whole multiclass stable. Illusions are the
					// deliberate exception above (targeted pet only). Do not "fix" this
					// to single-target.
	...
	for (buffslot = 0; buffslot < buff_count; buffslot++) {
		...
		if (IsValidOrSuppressedSpell(curbuf.spellid)) {
			...
			if (ret == -1) { // stop the spell
				...
				if (!will_overwrite && !IsDisciplineBuff(spell_id) && (IsShortDurationBuff(spell_id) == IsShortDurationBuff(buff_spellid))) {
					emptyslot = buffslot;
				}
...
//	buff->tic_timer = tic_timer.GetRemainingTime();
	...
	for(int i = 0; i < buff_count; ++i) {
		...
		if (buffs[i].spellid == SPELL_SUPPRESSED) {
			buff->entries[index].buff_slot = i;
			buff->entries[index].spell_id = RuleI(Custom, SuppressDebuffSpellID);
			buff->entries[index].tics_remaining = buffs[i].ticsremaining;
