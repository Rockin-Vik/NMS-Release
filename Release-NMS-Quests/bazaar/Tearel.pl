sub EVENT_SAY {
  ...
  my $favor_link      = quest::varlink(46779);
  ...
  if ($text=~/anchor yourself/i) {
    if ($group_flg) {
      ...
      plugin::NPCTell("If you can provide me with Five [".$favor_link."], I will [perform this ritual] for you to become more highly attuned to the Map.");
  ...
  if ($text=~/perform this ritual/i) {
    if ($group_flg) {
      ...
      if (plugin::SpendEmperorsFavor($client, 5)) {
        ...
        plugin::NPCTell("I'm sorry, $name, you do not have enough [".$favor_link."] available to you right now. When you have more...");
      }
    }
  }
}  
