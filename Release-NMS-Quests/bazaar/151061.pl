...
sub name_change_intro {
    ...
    plugin::NPCTell(
        ...
        plugin::num2en($name_change_cost) . " [" . plugin::EmperorsFavorLink() . "], or " .
        commaify($platinum_alt_cost * $name_change_cost) . "pp, I can help you claim a new identity."
    );

    plugin::NPCTell(
        "Do you want to [" . quest::saylink("confirm_name_change-1", 1, "pay with Emperor's Favor") . "] " .
...
sub name_change_confirm {
    ...
    my $success = $mode
        ? plugin::SpendEmperorsFavor($client, $name_change_cost)
...
sub race_change_intro {
    plugin::NPCTell(
        ...
        plugin::num2en($race_change_cost) . " [" . plugin::EmperorsFavorLink() . "], or " .
...
sub race_change_select {
    ...
    if (exists $races{$race_id} && $client->GetRace() != $race_id) {
        ...
        plugin::NPCTell(
            ...
            "Do you want to [" . quest::saylink("confirm_race_$race_id-1", 1, "pay with Emperor's Favor") . "] " .
...
sub race_change_confirm {
    ...
    my $success = $mode
        ? plugin::SpendEmperorsFavor($client, $race_change_cost)
...
sub deity_change_intro {
    ...
    $reply .= " With a mere " . plugin::num2en($gods_change_cost) . " [" . plugin::EmperorsFavorLink() . "], or " .
...
sub deity_change_select {
    ...
    if (exists $deities{$deity_id} && $client->GetDeity() != $deity_id) {
        ...
        plugin::NPCTell(
            ...
            "Do you want to [" . quest::saylink("confirm_deity_$deity_id-1", 1, "pay with Emperor's Favor") . "] " .
...
sub deity_change_confirm {
    my ($deity_id, $mode) = @_;
    my $success = $mode
        ? plugin::SpendEmperorsFavor($client, $gods_change_cost)
...
sub gender_change_intro {
    my $gender_message = "If you seek to embrace your " .
        ...
        "For a mere " . plugin::num2en($sex_change_cost) . " [" . plugin::EmperorsFavorLink() . "], or " .
    ...
    $gender_message .= " You have chosen to change your form to " .
                       ...
                       "Do you wish to [" .
                       quest::saylink("confirm_gender_$new_gender-1", 1, "pay with Emperor's Favor") .
                       "] or with [" .
...
sub gender_change_confirm {
    my ($gender_id, $mode) = @_;
    my $success = $mode
        ? plugin::SpendEmperorsFavor($client, $sex_change_cost)
...
sub pet_name_change_intro {
    plugin::NPCTell(
        ...
        "For a mere " . plugin::num2en($pet_name_reset_cost) . " [" . plugin::EmperorsFavorLink() . "], or " .
...
sub pet_name_change_select {
    ...
    plugin::NPCTell(
        ...
        quest::saylink("confirm_pet_name_change_$class_id-1", 1, "pay with Emperor's Favor") .
        "] or with [" .
...
sub pet_name_change_confirm {
    ...
    my $class_name = $class_map{$class_id};
    my $success = $mode
        ? plugin::SpendEmperorsFavor($client, $pet_name_reset_cost)
